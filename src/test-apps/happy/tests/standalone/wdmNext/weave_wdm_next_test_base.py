...
class weave_wdm_next_test_base(unittest.TestCase):
    ...
    def __validate_node_events(self, wdm_stress_check, stress_failures, data, client_gen=False, server_gen=False):
        """
    ...
    def __process_result(self, nodeA, nodeB, value, data):
        ...
        if not (self.client_faults or self.server_faults):
            if self.client_event_generator is not None:
                wdm_stress_check, stress_failures = self.__validate_node_events(wdm_stress_check, stress_failures, data, client_gen=True)

            if self.server_event_generator is not None:
                wdm_stress_check, stress_failures = self.__validate_node_events(wdm_stress_check, stress_failures, data, server_gen=True)
